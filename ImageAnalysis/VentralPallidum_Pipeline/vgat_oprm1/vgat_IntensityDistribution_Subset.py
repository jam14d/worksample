...
# Path to data
path_det = "/Users/jamieannemortel/Downloads/RawData_vgat/detection results"
os.chdir(path_det)

# Directory for saving plots
plots_dir = "/Users/jamieannemortel/Downloads/RawData_vgat/plots"
...
for f in filelist:
    ...
    # Subset the data for vgat_Pos only, or treat it as zero if not found
    Blue_posName = "vgat_Pos"
    Blue_posName_2 = "vgat_Pos: vgat_Neg"

    if Blue_posName in Read_Data['Name'].values or Blue_posName_2 in Read_Data['Name'].values:
        Cell = Read_Data[Read_Data['Name'].isin([Blue_posName, Blue_posName_2])]
    else:
        print(f"No {Blue_posName} or {Blue_posName_2} found in {f}. Treating as zero intensity data.")
        # If the `vgat_Pos` group is not found, we can assume zero values for the analysis
        Cell = Read_Data.copy()  # Keep the original data, but no relevant 'Blue_posName' entries
    ...
    if not Cell_filtered.empty:
        ...
        plt.title(f"Histogram of oprm1 mean signal intensity for {Blue_posName}")
        plt.xlabel("Intensity")
        plt.ylabel("Frequency")
        plt.savefig(os.path.join(plots_dir, f"histogram_{Blue_posName}_{f}.png"))
        plt.close()
        ...
        print(f"No valid data found for {Blue_posName} in {f}")
...
if custom_min_value_list:
    ...
    overall_values = pd.DataFrame({
        "Metric": ["Min Value", "Lower 25th Percentile Intensity", "50th Percentile Intensity", "Upper 75th Percentile Intensity", "Custom Max Value"],

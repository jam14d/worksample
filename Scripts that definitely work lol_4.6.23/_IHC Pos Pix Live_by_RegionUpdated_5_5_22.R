...
path.ano = "C://Analysis_Studies//22-170 Astellas_MTC-fibrosis_qp//annotation results_sl"
...
for(f in 1:length(filelist)){
  ...
  for(regionName in regionNames){
    if(regionName == 'A' | regionName == 'B' | regionName == 'C' ){
      ...
      positive_area_mm <- sum(Read.Data.region$fibrosis_1..Positive.area.µm.2) / 1000000
    
      annotation_area <- sum(Read.Data.region$Area.µm.2) / 1000000

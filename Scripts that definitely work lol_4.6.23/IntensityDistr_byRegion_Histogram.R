...
path.det = "D://23-159 Grace_qp//detections_binnedslabs"
path.ano = "D://23-159 Grace_qp//annotations_binnedslabs"
path.hist = "D://23-159 Grace_qp//histo"
...
for(f in 1:length(filelist)){
  ...
  for(regionName in unique(Read.Data$Parent)){
    if(regionName == 'Cerebellum' | regionName == 'Cortex'| regionName == 'Hippocampus'| regionName == 'Hypothalamus'| regionName == 'Thalamus' ){
      ...
      Positive <- subset(Read.Data, Read.Data$Class == "low" | Read.Data$Class == "medium" | Read.Data$Class == "high" )
      
      #histogram
      filename_histo <- gsub(".mrxs Detections.txt", "", filelist[f])
      setwd(path.hist)
      jpeg(file = paste(filename_histo, regionName, "Intensity.jpg"))
      hist(Positive$DAB..Nucleus..Mean, main = paste(filename_histo, regionName, ": Intensity"), xlab = "Cell Intensity Mean", xlim = c(0, 1.5), ylim = c(0, 15000))
      dev.off()
      setwd(path.det)
...
write.csv(DataDraft,paste("Data_BinnedbyRegion_Histogram",k,".csv"), row.names=FALSE)
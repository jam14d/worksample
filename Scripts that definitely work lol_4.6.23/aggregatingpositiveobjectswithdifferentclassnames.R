...
path.det = "C://Analysis_Studies//22-365 Boudless QP//detection results_hscore_10.28.22"
path.ano = "C://Analysis_Studies//22-365 Boudless QP//annotation results_hscore_10.28.22"
setwd(path.det)
...
for(f in 1:length(filelist)){
  ...
  posall <- subset(Read.Data, Read.Data$Class == "low" | Read.Data$Class == "medium" | Read.Data$Class == "high")
  ...
  posallArea <- sum(posall$Cell..Area) / 1000000 #division for conversion frum um^2 to mm^2
  ...
  posall_MeanIntensity <- mean(posall$Nucleus..DAB.OD.mean)
  ...
  DataDraft[k,2] <- nrow(posall) / anoArea
  DataDraft[k,3] <- nrow(posall)
  DataDraft[k,4] <- nrow(posall) / (Tot.Cells) 
  DataDraft[k,5] <- posall_MeanIntensity
  DataDraft[k,6] <- posallArea
  DataDraft[k,7] <- nrow(negative) / anoArea
  DataDraft[k,8] <- nrow(negative)
  DataDraft[k,9] <- nrow(negative) / (Tot.Cells) 
  DataDraft[k,10] <- negative_MeanIntensity
  DataDraft[k,11] <- negativeArea
  DataDraft[k,12] <- anoArea
  DataDraft[k,13] <- (Tot.Cells)
...
write.csv(DataDraft,paste("Data_10.31.22",f,".csv"))

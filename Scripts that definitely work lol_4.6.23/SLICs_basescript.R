...
path.det = "C://Analysis_Studies//22-160 LifeNet IHC_qp//detection results"
path.ano = "C://Analysis_Studies//22-160 LifeNet IHC_qp//annotation results"
setwd(path.det)
...
for(f in 1:length(filelist)){
  ...
  Read.Data$ROI..2.00.µm.per.pixel..DAB..Mean[is.nan(Read.Data$ROI..2.00.µm.per.pixel..DAB..Mean)] <- 0
  ...
  anoArea <- sum(realAnnotations$Area.�m.2)
  ...
  DataDraft[k,6] <- mean(Positive$ROI..2.00.µm.per.pixel..DAB..Mean)  #may need to update column name that's selected here
